       ...
       agent {
        ...
        stages
        {
          stage('Build App')
          {
            steps
             {
              git branch: 'main', url: 'https://github.com/arunkush5/openshift-jenkins-cicd.git'
              script {
          ...
          stage('Create Image Builder') {
            ...
            steps {
              script {
                openshift.withCluster() {
                  openshift.withProject() {
                    openshift.newBuild("--name=sample-app-jenkins-new", "--image-stream=ubi8/openjdk-8:latest", "--binary=true")
                  }
                }
              }
            }
